...
public class LogUtil {
    ...
    private static boolean fileIsExist() {
        ...
        if (sDStateString.equals(android.os.Environment.MEDIA_MOUNTED)) {
            try {
                ...
                File logDir = new File(SDFile.getAbsolutePath() + File.separator + Constants.PLAT_FOLDER);
                File logFlagFile = new File(SDFile.getAbsolutePath() + File.separator + Constants.PLAT_FOLDER + File.separator + "outdebug");
                File logFile = new File(SDFile.getAbsolutePath() + File.separator + Constants.PLAT_FOLDER + File.separator + "outdebuginfo.log");
                if (!logDir.exists()) {
                    logDir.mkdir();
                    return false;
                }
    ...
    private static void writeLogFromSDCard(String fileName, String funName, String msg) {
        ...
        if (sDStateString.equals(android.os.Environment.MEDIA_MOUNTED)) {
            try {
                File SDFile = android.os.Environment.getExternalStorageDirectory();
                File myDir = new File(SDFile.getAbsolutePath() + File.separator + Constants.PLAT_FOLDER);
                File myFile = new File(SDFile.getAbsolutePath() + File.separator + Constants.PLAT_FOLDER + File.separator + "outdebuginfo.log");
                if (!myDir.exists()) {
                    myDir.mkdir();
                ...
                outputStream = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(myFile, true)));
