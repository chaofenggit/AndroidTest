...
import com.altopay.lib.utils.AESHelper;
import com.altopay.lib.utils.EncryptUtil;
...
//        this.aesKey = new byte[16];// TODO 生成一个随机字符串
...
//        byte[0]+toBytes(id)+toBytes(lengthOf(enData))+ enData+ toBytes(lengthOf(key))+key+ toBytes(lengthOf(sign))+sign
    ...
    public byte[] decode(byte[] bytes) {
        ...
        int code = input.read();
        if (code != 0) {
            LogUtil.e("Exception @ SecurityCodeBody code: " + code);
